...
class _ExchangeScreenState extends State<ExchangeScreen> {
  ...
  static const List<_ConnectPartner> _dummyPartners = [
    _ConnectPartner(
      name: 'EcoFuel Goa Pvt Ltd',
      city: 'Panaji',
      offer: 'Buys used cooking oil',
      contact: '+91 98765 11021',
    ),
    _ConnectPartner(
      name: 'Zuari Bio Composters',
      city: 'Margao',
      offer: 'Collects organic waste',
      contact: '+91 98765 44210',
    ),
    _ConnectPartner(
      name: 'GreenPack Circular',
      city: 'Mapusa',
      offer: 'Takes cardboard/plastic for recycling',
      contact: '+91 99887 33102',
    ),
    _ConnectPartner(
      name: 'Coastal Feed Network',
      city: 'Vasco',
      offer: 'Takes bakery surplus for feed channel',
      contact: '+91 90000 41054',
    ),
  ];
  ...
  Widget build(BuildContext context) {
    if (widget.tourismController != null) {
      return AnimatedBuilder(
        ...
        builder: (_, __) =>
            _buildExchangeWithCC(context, widget.tourismController!),
      );
    }
        ...
        child: SafeArea(
          child: ListView(
            ...
            children: [
              ...
              const Text(
                'Trade waste into value & monetize your carbon footprint',
                    ...
                    children: [
                      ...
                      SizedBox(
                        ...
                        child: ElevatedButton.icon(
                          ...
                          label: const Text(
                            'Analyze My Carbon Credit Position',
                          ),
                ...
                GlassCard(
                  ...
                  child: Column(
                    children: [
                      ...
                      const SizedBox(height: 16),
                      const Text(
                        'Analyzing your carbon credit position...',
                        style: TextStyle(
                          color: AppTheme.textPrimary,
                          fontSize: 14,
                      ...
                      const SizedBox(height: 4),
                      const Text(
                        'Checking India CCTS • Verra VCS • Gold Standard • VCM pricing...',
                        style: TextStyle(
                          color: AppTheme.textSecondary,
                          fontSize: 11,
                        ),
              ...
              // ── RESULTS ──
              if (cc != null) ...[
                if (cc['analysis_confidence'] != null)
                  _buildConfidenceBanner(cc['analysis_confidence']),
                const DisclaimerBanner(
                  disclaimerKey: 'carbon_credit_disclaimer',
                ),
                ...
                SizedBox(
                  ...
                  child: OutlinedButton.icon(
                    ...
                    onPressed: c.isLoadingCarbonCredit
                        ? null
                        : c.generateCarbonCreditAnalysis,
                  ),
                ),
              ],
              ...
              const SizedBox(height: 4),
              const Text(
                'Connect with nearby businesses to trade waste into resources',
                  ...
                  child: GlassCard(
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 6),
                        Text(
                          '${m.nearbyBusinesses} businesses nearby can use your waste',
                          style: const TextStyle(
                        ...
                        GradientButton(
                          label: 'Connect',
                          onPressed: () => _showConnectDialog(context, m.title),
                          width: double.infinity,
                        ),
                      ],
                    ),
          ...
          Container(
            ...
            child: Row(
              ...
              children: [
                _statColumn(
                  'Annual CO₂',
                  '${_safeDiv(cc['annual_emissions_kg'], 1000)} t',
                  Icons.cloud,
                  color,
                ),
                Container(
                ...
                _statColumn(
                  'Position',
                  _trimNetPosition(cc['net_position']?.toString() ?? '—'),
                  Icons.swap_vert,
                  color,
                ),
                Container(
                ...
                _statColumn(
                  'Offset Cost',
                  _trimOffsetCost(cc['offset_cost']?.toString() ?? '—'),
                  Icons.payments,
                  color,
                ),
              ],
  ...
  Widget _buildCreditOpportunities(Map<String, dynamic> cc) {
    ...
    if (opps == null || opps is! List || opps.isEmpty)
    ...
    return GlassCard(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 4),
          const Text(
            'Ways your business can generate or earn carbon credits',
          ...
          ...opps.map<Widget>((opp) {
            ...
            final num potentialCredits = (potentialCreditsRaw is num)
                ? potentialCreditsRaw
                : double.tryParse(potentialCreditsRaw.toString()) ?? 0;
            final isBiochar =
                o['type']?.toString().toLowerCase().contains('biochar') ??
                false;

            Widget warningChip;
            if (potentialCredits < 50) {
              warningChip = _alertChip(
                "⚠️ Too small for any carbon market. Focus on cost savings instead.",
                AppTheme.accentRed,
              );
              ...
              warningChip = Column(
                ...
                children: [
                  _alertChip(
                    "Small project — only viable via aggregator. Solo registration not possible at this scale.",
                    AppTheme.warning,
                  ),
                  const SizedBox(height: 4),
                  const Text(
                    "Connect with Varaha (varaha.earth) or Boomitra (boomitra.com) for aggregated farmer/land projects.",
                    style: TextStyle(
                      color: AppTheme.textSecondary,
                      fontSize: 10,
                    ),
              ...
              warningChip = _alertChip(
                "Potentially viable for independent registration. Get a professional assessment.",
                AppTheme.emerald,
              );
            }
                ...
                children: [
                  ...
                  Wrap(
                    ...
                    children: [
                      _chip(
                        Icons.bar_chart,
                        '$potentialCredits tonnes CO₂e/year',
                      ),
                      _chip(
                        Icons.currency_rupee,
                        o['estimated_revenue']?.toString() ?? '',
                      ),
                  ...
                  const SizedBox(height: 8),
                  const Text(
                    "(Estimated. Does not account for ~₹40L–₹4Cr project development and verification costs for independent registration.)",
                    style: TextStyle(
                      color: AppTheme.textSecondary,
                      fontSize: 10,
                  ...
                  if (isBiochar) ...[
                    const SizedBox(height: 8),
                    const Text(
                      "Biochar credits (\$100–200/tonne) require a pyrolysis unit (capital cost: ₹15L–₹1Cr), specific temperature-controlled burning, and registration via Puro.earth registry. This is a medium-term opportunity, not immediately actionable.",
                      style: TextStyle(
  ...
  Widget _buildActionPlan(Map<String, dynamic> cc) {
    ...
    double progress = _actionItems.isEmpty
        ? 0
        : completedCount / _actionItems.length;
        ...
        children: [
          ...
          Row(
            children: [
              ...
              Text(
                '[$completedCount/${_actionItems.length}] completed',
                style: TextStyle(
                  color: progressColor,
                  fontSize: 12,
                      ...
                      children: [
                        ...
                        if (item.isCompleted && item.completedAt != null)
                          Padding(
                            ...
                            child: Text(
                              'Completed ${item.completedAt!.day}/${item.completedAt!.month}/${item.completedAt!.year}',
                              style: const TextStyle(
                                color: AppTheme.emerald,
                                fontSize: 10,
                              ),
    ...
    return GlassCard(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 4),
          const Text(
            'Government programs relevant to your carbon position',
            style: TextStyle(color: AppTheme.textSecondary, fontSize: 11),
          ),
          const SizedBox(height: 12),
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (scheme['potential_benefit'] != null) ...[
                          const SizedBox(height: 4),
                          Text(
                            'Potential: ${scheme['potential_benefit']}',
                            style: const TextStyle(
                              color: AppTheme.lime,
                              fontSize: 12,
                              fontWeight: FontWeight.w600,
      ...
      child: Column(
        ...
        children: [
          ...
          Container(
            ...
            child: const Column(
              ...
              children: [
                ...
                SizedBox(height: 4),
                Text(
                  "Renewable energy (solar/wind) credits are increasingly rejected by ICVCM due to additionality concerns. Focus on nature-based or high-tech removal (like Biochar) for premium pricing.",
                  style: TextStyle(color: AppTheme.textSecondary, fontSize: 11),
                ),
              ],
            ),
          ...
          const Align(
            ...
            child: Text(
              "Data as of March 2025 • Source: ICVCM & Verra",
              style: TextStyle(
                color: AppTheme.textSecondary,
                fontSize: 10,
                fontStyle: FontStyle.italic,
  ...
  Widget _buildConfidenceBanner(Map<String, dynamic> conf) {
    ...
    if (score >= 8) {
      ...
      color = AppTheme.warning;
      title = "Moderate Confidence — Verify with expert";
    } else {
      color = AppTheme.accentRed;
      title =
          "Low Confidence — AI has limited data on this business type. Do not rely on these figures.";
    }
      ...
      children: [
        ...
        if (!isStandard)
          Container(
            ...
            child: const Text(
              "Your business type is outside common carbon market categories. The estimates below are experimental. Consult a carbon specialist before making any decisions.",
              style: TextStyle(
                color: AppTheme.accentRed,
                fontSize: 11,
                fontWeight: FontWeight.bold,
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => Padding(
        ...
        child: Column(
          ...
          children: [
            const Text(
              "Next Steps for a Real Carbon Credit Assessment",
              style: TextStyle(
                color: AppTheme.textPrimary,
                fontSize: 18,
                fontWeight: FontWeight.bold,
            ...
            const SizedBox(height: 16),
            _linkRow(
              "Contact BEE (Bureau of Energy Efficiency):",
              "https://bee.gov.in",
            ),
            const SizedBox(height: 12),
            _linkRow(
              "For voluntary markets: Contact an accredited VVB like SGS, Bureau Veritas, or SCS Global",
              "https://verra.org/validation-verification",
            ),
            const SizedBox(height: 12),
            _linkRow(
              "For farmer/land projects in India: Varaha Climate or Boomitra aggregate small projects",
              "https://varaha.earth",
            ),
            const SizedBox(height: 16),
            const Text(
              "• Estimated consultation cost: ₹50,000–₹5,00,000 depending on project size",
              style: TextStyle(color: AppTheme.textSecondary, fontSize: 13),
            ),
            const SizedBox(height: 6),
            const Text(
              "• Timeline to first credit issuance: 12–24 months",
              style: TextStyle(color: AppTheme.textSecondary, fontSize: 13),
            ),
            const SizedBox(height: 24),
            SizedBox(
      ...
      builder: (ctx) => Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 4),
            const Text(
              "The AI provides directional estimates based on standard industry averages. It CANNOT certify projects, verify exact footprints, or guarantee carbon credit market access. Real registration requires an accredited third-party audit.",
              style: TextStyle(color: AppTheme.textSecondary, fontSize: 13),
            ),
            const SizedBox(height: 12),
            const Text(
              "How emission factors are sourced:",
              style: TextStyle(
                color: AppTheme.emerald,
                fontSize: 14,
                fontWeight: FontWeight.bold,
            ...
            const SizedBox(height: 4),
            const Text(
              "Data is sourced from the India GHG Platform, IPCC AR6, and India BEE PAT Scheme guidelines.",
              style: TextStyle(color: AppTheme.textSecondary, fontSize: 13),
            ),
            const SizedBox(height: 12),
            const Text(
            ...
            const SizedBox(height: 4),
            const Text(
              "MRV is the rigorous process required by registries (like Verra or Gold Standard) to prove your carbon reduction is real, quantifiable, and permanent. Without MRV, you cannot sell credits.",
              style: TextStyle(color: AppTheme.textSecondary, fontSize: 13),
            ),
            const SizedBox(height: 16),
  ...
  String _safeDiv(dynamic val, int divisor) {
    if (val == null) return '—';
    final num = val is int
        ? val.toDouble()
        : (val is double ? val : double.tryParse(val.toString()) ?? 0);
        ...
        child: Container(
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              const SizedBox(height: 4),
              const Text(
                'Dummy local partners to connect with',
                style: TextStyle(color: AppTheme.textSecondary, fontSize: 12),
              ),
              const SizedBox(height: 10),
              ..._dummyPartners.map((partner) {
                return Padding(
...
class _ConnectPartner {
  final String name;
  final String city;
  final String offer;
  final String contact;
  const _ConnectPartner({
    required this.name,
    required this.city,
    required this.offer,
