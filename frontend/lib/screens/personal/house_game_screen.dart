                    ...
                    children: [
                      const Expanded(
                        child: Column(
                          ...
                          children: [
                            Text('Green House', style: TextStyle(color: AppTheme.textPrimary, fontSize: 24, fontWeight: FontWeight.w700)),
                            SizedBox(height: 2),
                            Text('Upgrade your eco home', style: TextStyle(color: AppTheme.textSecondary, fontSize: 13)),
                          ],
                        ),
                      ),
                      GlassCard(
...
class _HousePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final rng = Random(42);
    for (int i = 0; i < 30; i++) {
      canvas.drawCircle(Offset(rng.nextDouble() * w, rng.nextDouble() * h * 0.45), rng.nextDouble() * 1.5, starPaint);
    }

    // Ground
    final groundY = h * 0.72;
    final groundGradient = LinearGradient(
      colors: [hasGarden ? const Color(0xFF14532D) : const Color(0xFF1A2E1A), const Color(0xFF0D1A0D)],
      begin: Alignment.topCenter,
      end: Alignment.bottomCenter,
    ).createShader(Rect.fromLTWH(0, groundY, w, h - groundY));
    canvas.drawRect(Rect.fromLTWH(0, groundY, w, h - groundY), Paint()..shader = groundGradient);
    ...
    if (hasGarden) {
      ...
      for (double gx = w * 0.05; gx < w * 0.95; gx += 14) {
        canvas.drawLine(Offset(gx, groundY), Offset(gx - 3, groundY - 8), grassPaint);
        canvas.drawLine(Offset(gx + 4, groundY), Offset(gx + 4, groundY - 10), grassPaint);
        canvas.drawLine(Offset(gx + 8, groundY), Offset(gx + 11, groundY - 7), grassPaint);
      }
      // Flower
    ...
    ).createShader(Rect.fromLTWH(houseLeft, houseTop, houseWidth, houseBottom - houseTop));

    final wallPaint = Paint()..shader = wallGradient;
    canvas.drawRect(Rect.fromLTWH(houseLeft, houseTop, houseWidth, houseBottom - houseTop), wallPaint);
    canvas.drawRect(
    ...
    ).createShader(Rect.fromLTWH(houseLeft, h * 0.12, houseWidth, houseTop - h * 0.12));
    ...
    // Windows
    _drawWindow(canvas, Offset(houseLeft + houseWidth * 0.2, houseTop + (houseBottom - houseTop) * 0.2), 22, 18);
    _drawWindow(canvas, Offset(houseLeft + houseWidth * 0.7, houseTop + (houseBottom - houseTop) * 0.2), 22, 18);
    ...
    // Doorknob
    canvas.drawCircle(Offset(doorLeft + 24, doorTop + 30), 3, Paint()..color = AppTheme.lime);
    ...
    // Level badge
    final purchasedCount = [hasSolar, hasGarden, hasEV, hasWater, hasBike].where((b) => b).length;
    _drawLevelBadge(canvas, Offset(w - 20, 16), purchasedCount);
  }

  void _drawSolarPanels(Canvas canvas, Size size, double houseLeft, double roofTop, double houseTop) {
    final panelPaint = Paint()..color = const Color(0xFF1E40AF).withValues(alpha: 0.9);
    ...
    // Left side of roof
    final panels = [
      Rect.fromLTWH(size.width * 0.34, roofTop + (houseTop - roofTop) * 0.45, 24, 14),
      Rect.fromLTWH(size.width * 0.34 + 26, roofTop + (houseTop - roofTop) * 0.4, 24, 14),
    ];
    for (final r in panels) {
      canvas.drawRect(r, panelPaint);
      canvas.drawRect(r, Paint()..color = Colors.blue.shade300.withValues(alpha: 0.3)..style = PaintingStyle.stroke..strokeWidth = 0.5);
      // Grid lines
      canvas.drawLine(Offset(r.left + r.width / 2, r.top), Offset(r.left + r.width / 2, r.bottom), linePaint);
      canvas.drawLine(Offset(r.left, r.top + r.height / 2), Offset(r.right, r.top + r.height / 2), linePaint);
    }
    // Glow dot
    canvas.drawCircle(Offset(panels[0].right + 1, panels[0].top - 2), 3, glowPaint);
  }
  ...
  void _drawWindow(Canvas canvas, Offset topLeft, double w, double h) {
    ...
    canvas.drawRRect(RRect.fromRectAndRadius(rect, const Radius.circular(3)),
        Paint()..color = AppTheme.lime.withValues(alpha: 0.5)..style = PaintingStyle.stroke..strokeWidth = 1.5);
    // Cross
    final cross = Paint()..color = AppTheme.lime.withValues(alpha: 0.3)..strokeWidth = 1;
    canvas.drawLine(Offset(topLeft.dx + w / 2, topLeft.dy), Offset(topLeft.dx + w / 2, topLeft.dy + h), cross);
    canvas.drawLine(Offset(topLeft.dx, topLeft.dy + h / 2), Offset(topLeft.dx + w, topLeft.dy + h / 2), cross);
  }
  ...
  void _drawEVCharger(Canvas canvas, Offset pos, double groundY) {
    ...
    // Box
    canvas.drawRRect(
      RRect.fromRectAndRadius(Rect.fromLTWH(pos.dx - 8, pos.dy, 22, 28), const Radius.circular(4)),
      Paint()..color = const Color(0xFF1F2937),
    );
    canvas.drawRRect(
      RRect.fromRectAndRadius(Rect.fromLTWH(pos.dx - 8, pos.dy, 22, 28), const Radius.circular(4)),
      Paint()..color = AppTheme.lime.withValues(alpha: 0.6)..style = PaintingStyle.stroke..strokeWidth = 1.5,
    );
    // Lightning bolt
    final boltPath = Path();
    ...
    cablePath.quadraticBezierTo(pos.dx - 20, pos.dy + 35, pos.dx - 15, groundY - 3);
    canvas.drawPath(cablePath, Paint()..color = AppTheme.lime.withValues(alpha: 0.7)..strokeWidth = 2..style = PaintingStyle.stroke);
  }

  void _drawWaterTank(Canvas canvas, Offset pos) {
    // Tank body
    canvas.drawRRect(
      RRect.fromRectAndRadius(Rect.fromLTWH(pos.dx, pos.dy, 28, 22), const Radius.circular(4)),
      Paint()..color = const Color(0xFF1E3A5F),
    );
    canvas.drawRRect(
      RRect.fromRectAndRadius(Rect.fromLTWH(pos.dx, pos.dy, 28, 22), const Radius.circular(4)),
      Paint()..color = Colors.blue.shade300.withValues(alpha: 0.6)..style = PaintingStyle.stroke..strokeWidth = 1.5,
    );
    // Water level
    canvas.drawRRect(
      RRect.fromRectAndRadius(Rect.fromLTWH(pos.dx + 2, pos.dy + 10, 24, 10), const Radius.circular(3)),
      Paint()..color = Colors.blue.shade400.withValues(alpha: 0.4),
    );
    // Pipe
    canvas.drawRect(Rect.fromLTWH(pos.dx + 11, pos.dy + 22, 6, 8), Paint()..color = const Color(0xFF374151));
    // Droplet
    final dropPath = Path();
    dropPath.moveTo(pos.dx + 14, pos.dy + 32);
    dropPath.quadraticBezierTo(pos.dx + 10, pos.dy + 38, pos.dx + 14, pos.dy + 40);
    dropPath.quadraticBezierTo(pos.dx + 18, pos.dy + 38, pos.dx + 14, pos.dy + 32);
    canvas.drawPath(dropPath, Paint()..color = Colors.blue.shade300.withValues(alpha: 0.8));
  }
  ...
  void _drawBike(Canvas canvas, Offset pos) {
    ...
    canvas.drawCircle(Offset(pos.dx + 12, pos.dy - 8), 10, bikePaint);
    // Frame
    canvas.drawLine(Offset(pos.dx - 12, pos.dy - 8), Offset(pos.dx, pos.dy - 20), bikePaint);
    canvas.drawLine(Offset(pos.dx, pos.dy - 20), Offset(pos.dx + 12, pos.dy - 8), bikePaint);
    canvas.drawLine(Offset(pos.dx - 12, pos.dy - 8), Offset(pos.dx + 12, pos.dy - 8), bikePaint);
    canvas.drawLine(Offset(pos.dx - 2, pos.dy - 20), Offset(pos.dx + 2, pos.dy - 20), bikePaint..strokeWidth = 1.5);
    // Handlebar
    canvas.drawLine(Offset(pos.dx + 9, pos.dy - 20), Offset(pos.dx + 15, pos.dy - 20),
        bikePaint..color = AppTheme.lime..strokeWidth = 2);
    // Seat
    canvas.drawLine(Offset(pos.dx - 5, pos.dy - 22), Offset(pos.dx + 2, pos.dy - 22),
        bikePaint..color = AppTheme.lime..strokeWidth = 2.5);
  }

  void _drawTree(Canvas canvas, Offset base, bool lush) {
    final trunkPaint = Paint()..color = const Color(0xFF4A2C0A);
    canvas.drawRect(Rect.fromLTWH(base.dx - 5, base.dy - 30, 10, 30), trunkPaint);
    ...
    canvas.drawCircle(Offset(base.dx + 12, base.dy - 30), 14, leafPaint);
    if (lush) {
      canvas.drawCircle(Offset(base.dx, base.dy - 52), 12, Paint()..color = AppTheme.lime.withValues(alpha: 0.6));
    }
  }

  void _drawFlower(Canvas canvas, Offset base) {
    final petalPaint = Paint()..color = AppTheme.lime.withValues(alpha: 0.8);
    final centerPaint = Paint()..color = const Color(0xFFEAB308);
    for (int i = 0; i < 5; i++) {
      final angle = (i / 5) * 2 * pi;
      canvas.drawCircle(Offset(base.dx + cos(angle) * 5, base.dy + sin(angle) * 5 - 8), 4, petalPaint);
    }
    canvas.drawCircle(Offset(base.dx, base.dy - 8), 4, centerPaint);
    canvas.drawLine(Offset(base.dx, base.dy - 4), Offset(base.dx, base.dy), Paint()..color = const Color(0xFF166534)..strokeWidth = 1.5);
  }

  void _drawCelebration(Canvas canvas, Size size, double progress) {
    final rng = Random(12);
    final colors = [AppTheme.emerald, AppTheme.lime, const Color(0xFFEAB308), Colors.white];
    for (int i = 0; i < 18; i++) {
  ...
  void _drawLevelBadge(Canvas canvas, Offset pos, int count) {
    final stars = count;
    final color = stars >= 4 ? AppTheme.lime : stars >= 2 ? AppTheme.emerald : AppTheme.textSecondary;
    canvas.drawCircle(pos, 16, Paint()..color = color.withValues(alpha: 0.15));
    canvas.drawCircle(pos, 16, Paint()..color = color.withValues(alpha: 0.4)..style = PaintingStyle.stroke..strokeWidth = 1.5);
    final tp = TextPainter(
      text: TextSpan(text: 'Lv$stars', style: TextStyle(color: color, fontSize: 9, fontWeight: FontWeight.w700)),
...
class _ShopCard extends StatefulWidget {
  ...
  const _ShopCard({required this.item, required this.canAfford, required this.onBuy});
...
class _ShopCardState extends State<_ShopCard> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      onTapDown: (_) => widget.canAfford && !widget.item.purchased ? _tapController.forward() : null,
      onTapUp: (_) {
      ...
      child: AnimatedBuilder(
        ...
        child: GlassCard(
          borderColor: widget.item.purchased ? AppTheme.emerald.withValues(alpha: 0.5) : null,
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text(widget.item.icon, style: const TextStyle(fontSize: 36)),
              const SizedBox(height: 8),
              Text(widget.item.name, style: const TextStyle(color: AppTheme.textPrimary, fontSize: 14, fontWeight: FontWeight.w700)),
              const SizedBox(height: 4),
              Text(widget.item.description, style: const TextStyle(color: AppTheme.textSecondary, fontSize: 11), maxLines: 2),
              const Spacer(),
              if (widget.item.purchased)
                Container(
                  ...
                  child: const Row(
                    ...
                    children: [
                      ...
                      SizedBox(width: 4),
                      Text('Owned', style: TextStyle(color: AppTheme.emerald, fontSize: 12, fontWeight: FontWeight.w600)),
                    ],
                  ),
                )
              else
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: widget.canAfford ? AppTheme.emerald.withValues(alpha: 0.2) : AppTheme.surface,
                    borderRadius: BorderRadius.circular(8),
                    border: Border.all(color: widget.canAfford ? AppTheme.emerald : Colors.transparent),
                  ),
                  child: Row(
                    mainAxisSize: MainAxisSize.min,
                    children: [
                      ...
                      Text(
                        '${widget.item.cost}',
                        style: TextStyle(
                          color: widget.canAfford ? AppTheme.emerald : AppTheme.textSecondary,
                          fontSize: 12,
