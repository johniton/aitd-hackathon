...
class DeviceFootprintService {
  ...
  Future<DeviceUsageSnapshot?> captureSnapshot() async {
    ...
    if (connectivity == ConnectivityResult.wifi) {
      bytes[NetworkInterface.wifi] = 0.5 * _bytesPerGb; // Estimated 500MB/hr
    } else if (connectivity == ConnectivityResult.mobile) {
      bytes[NetworkInterface.cellular4g] = 0.1 * _bytesPerGb; // Estimated 100MB/hr
    }
  ...
  DeviceFootprintResult calculate(DeviceUsageSnapshot snapshot) {
    ...
    final networkBytes = snapshot.bytesTransferred.values.fold(0.0, (a, b) => a + b);
